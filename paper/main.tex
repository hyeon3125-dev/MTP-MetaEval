...
assumed-not-earned.} We close with a live application that is a \emph{metrology}
result, not a saturation verdict: ``capability saturation'' is undefined without a
cross-vendor capability metric, and none exists. On the one axis we do measure
(everyday-text Arena preference) adjacent frontier models tie (top-10 within
14~Elo, $\sim$51\% blind preference), but that axis is not capability: the same
frontier spans $\sim$256~Elo on coding, and the multimodal, agentic, and
vendor-specific axes---and even the ``everyday'' axis itself---are unmeasured. So
both the pro- and the anti-saturation position are assumed-not-earned. The
contribution is the cross-domain synthesis and a pre-registered, fabrication-averse
protocol that reports its own negatives---not new theory in any single field.
\end{abstract}
...
\section{Application: the capability-measurement problem}\label{sec:llm}
This section is not a verdict on whether LLMs have saturated. It is the sharper,
\emph{metrology} claim: ``capability saturation'' is undefined without a
cross-vendor capability metric, and none exists, so both the pro- and the
anti-saturation position are assumed-not-earned. As in physics, an unmeasurable
quantity is an undefined one. Applying the overhead boundary to model generations
(floor $=$ user-perceptible difference) is the lens that surfaces this: the verdict
is clean only on the one axis we instrument, and that axis is not capability. The
one exact ingredient is LMArena's blind win-rate map \citep{chiang2024},
...
\paragraph{The one axis we measure --- everyday-text preference --- has gone flat.}
That axis is not capability. The real Arena overall/text top-10 (June 2026) spans
$14~\elo$ (1510 down to 1496), so the largest adjacent gap is $\sim$7~\elo, a
$\sim$51\% blind preference---below the $35~\elo$ floor. The marginal gain per
generation on this axis has crossed the floor: GPT-3.5$\to$GPT-4 was $\sim$100~\elo
(earned); 2025--26 adjacent gaps are 4--14~\elo \citep{kaplan2020,hoffmann2022}.
...
\paragraph{A third level: the proxies are prior-calibrated too.} A within-speaker
case study (one author, casual vs technical text, derived metrics only) shows even
the everyday \emph{proxies} are not neutral. Per-proxy, only \emph{surface-lexicon}
measures (slang and jargon density) separate casual from technical text; every
\emph{register-complexity} measure---lexical diversity, sentence length,
compressibility---fails to separate. For this speaker ``everyday'' is not a lower
cognitive register, only a different surface vocabulary; the population prior
``everyday speech $=$ low complexity'' is false here. So the instrument that
measures whether structure is earned is \emph{itself} prior-calibrated and can be
assumed-not-earned---the same recursion as the detector self-test of
\S\ref{sec:earned}. The law applies to its own measuring tools, at every level.
...
reframing of the scaling debate as a capability-measurement (metrology) problem
